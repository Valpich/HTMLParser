            ...
            public void mouseClicked( MouseEvent e ) {
                if ( getID() != null && !txtCookie.getText().equals( "Saisir le cookie" ) ) {
                    try {
                        ...
                        getDb().ajouterEtudiant( getNom(), Integer.parseInt( getID() ),
                        ...
                        if ( !txtCookie.getText().equals( "Saisir le cookie" ) ) {
                            for ( Entry<String, Integer> element : map.entrySet() ) {
                                if ( test == true ) {
                                    ...
                                    try {
                                        StringBuffer resultat = RequestTools.sendGet( "webvpn=" + txtCookie.getText(),
                                                element.getValue().toString() );
                                        float sommeNotes = 0;
                                        float sommeCoefficients = 0;
                                        float notes[] = new float[30];
                                        float coefs[] = new float[30];
                                        int i = 0;
                                        for ( String tmp : NOM_UE ) {
                                            float coefficient = ParsingTools.recupererCoefficient( resultat, tmp );
                                            float note = ParsingTools.recupererNote( resultat, tmp );
                                        ...
                                        if ( i == 13 )
                                            db.ajouterEtudiantComplet( element.getValue() , element.getKey(), coefs[0],
                                                    notes[0],
                                                    coefs[1],
                                                    notes[1], coefs[2], notes[2], coefs[3], notes[3], coefs[4],
                                                    notes[4], coefs[5],
                                                    notes[5], coefs[6], notes[6], coefs[7], notes[7], coefs[8],
                                                    notes[8], coefs[9],
                                                    notes[9], coefs[10], notes[10], coefs[11], notes[11], coefs[12],
                                                    notes[12] );
                                        int rowCount = getTableModele().getRowCount();
                                        for ( int j = rowCount - 1; j >= 0; j-- ) {
                                            getTableModele().removeRow( j );
                                        }
                                        getTableModele()
                                                .addRow( new Object[] { "Parcours actuel", element.getKey(),
                                                        sommeNotes / sommeCoefficients } );
                                        getDb().ajouterEtudiant( element.getKey(),
                                                Integer.parseInt( element.getValue().toString() ),
                                                sommeNotes / sommeCoefficients );
                                    } catch ( Exception e1 ) {
                                        e1.printStackTrace();
                                    }
